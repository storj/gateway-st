...
func TestBucketNotificationConfig_ValidationErrors(t *testing.T) {
	...
	runTestWithEventing(t, func(t *testing.T, ctx context.Context, layer minio.ObjectLayer, project *uplink.Project) {
		...
		// Test that multiple Topic configurations are rejected
		config = &event.Config{
			TopicList: []event.Topic{
				{},
				{},
			},
		}
		...
		var invalidArg minio.InvalidArgument
		require.ErrorAs(t, err, &invalidArg)
		assert.Equal(t, testBucket, invalidArg.Bucket)
		assert.Contains(t, invalidArg.Err.Error(), "only a single topic destination is supported per bucket")
	})
}
